...
const ActiveQuiz: React.FC<ActiveQuizProps> = ({ quiz, studentAnswers, onEndQuiz }) => {
  ...
  const [quizParticipants, setQuizParticipants] = useState<{id: string, name: string}[]>([]);
  ...
  useEffect(() => {
    ...
    // Get unique students who have participated in THIS quiz only - using Set for better uniqueness
    ...
    quizAnswers.forEach(answer => {
      if (!uniqueStudents.has(answer.studentId)) {
        uniqueStudents.add(answer.studentId);
        participantMap.set(answer.studentId, answer.studentName);
      }
    });
        ...
        {quizParticipants.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
            {quizParticipants.map((participant) => {
              const hasAnsweredCurrentQuestion = answersForCurrentQuestion.some(a => a.studentId === participant.id);
              return (
                <Card key={participant.id} className={`p-2 flex items-center space-x-2 ${hasAnsweredCurrentQuestion ? 'bg-green-50 border-green-200' : ''}`}>
